...
Spawn.prototype.fn_build_walls_and_roads = function() {
    ...
    if (!this.room.memory.walls) {
        ...
        for (var y = 2; y < 47; y++) {
           for (var x = 2; x < 47; x++) {
                ...
                switch(obj[0].type) {
                    ...
                    case 'structure':
                    case 'mineral':
                        if (obj[0].structure && obj[0].structure.structureType && obj[0].structure.structureType == 'road' ||
                        obj[0].structure && obj[0].structure.structureType && obj[0].structure.structureType == 'extension') {
                            continue;
                        }
                        objects.push([x, y]);
                    break;
        ...
        if (!this.room.memory.wall_project) {
            ...
            for (var y = 2; y < 47; y++) {
                for (var x = 2; x < 47; x++) {
                    ...
                    switch (obj[0].type) {
                        ...
                        case 'structure':
                        case 'mineral':
                            if (obj[0].structure && obj[0].structure.structureType != 'road') {
            ...
            ///
            ///
            wall_project = this.fn_exits_from_box(wall_project, wall_borders);
            ///
            ///
...
Spawn.prototype.fn_exits_from_box = function(wall_project, wall_borders) {
   // console.log(JSON.stringify(wall_borders));
...
Spawn.prototype.fn_check_csites = function(path) {
    for (var i in path) {
        ...
        if (what[0].type != 'constructionSite' && what[0].type != 'source' && what[0].type != 'structure') {
            console.log("ERROR " + what[0].type + " " + what[0].terrain)
            return true;
        }
    }
    return false;
...
Spawn.prototype.fn_create_construction_sites = function(path, construction) {
    ...
    for (var index in path) {
        ...
        if (this.room.lookForAt('structure', roomPosition).length == 0 && 
        this.room.lookForAt('constructionSite', roomPosition).length == 0) {
            console.log(item.x + " " + item.y + ' Needed to be built')
