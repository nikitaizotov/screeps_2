...
var routines = {
    ...
    fn_room_connect: function(){
        // Todo: Needs refactor, eating too much CPU.
        if (Game.time % 130) {
            return;
        }
    ...
    fn_check_creep_population: function() {
        ...
        for (var index_spawns in spawns) {
            ...
            var harvesters = _.filter(Game.creeps, function(creep) { return creep.memory.temp_role == 'harvester' && creep.memory.home_room == spawn_obj.room.name; });
            var builders = _.filter(Game.creeps, function(creep) { return creep.memory.temp_role == 'builder' && creep.memory.home_room == spawn_obj.room.name; });
            // Spawn new creeps if needed,
            for (var unit in spawn_obj.memory.units) {
                var built = _.filter(Game.creeps, function(creep) { return creep.memory.temp_role == unit && creep.memory.home_room == spawn_obj.room.name; });
